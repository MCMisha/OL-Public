...
public class AdminContactSectionRepository : GenericRepository<Section>, IAdminAboutSectionRepository, IAdminContactSectionRepository
{
    public AdminContactSectionRepository(AppDbContext context) : base(context) { }

    public new async Task<IEnumerable<Section>> GetAllAsync()
    {
        return await _context.Sections.Where(s => s.Type == SectionType.Contact).ToListAsync();
    }
    ...
    public async Task<Section?> SetMainAsync(Section section)
    {
        ...
        if (section.IsMain)
        {
            var oldMainSections = await _context.Sections
                .Where(s => s.Id != section.Id && s.IsMain && s.Type == section.Type)
                .ToListAsync();
